...
public class SAT {
	...
	private boolean DPLL(CNFFormula formula, HashMap<Integer, Literala> baliodunLiteralak, HashMap<Integer, Literala> baliogabekoLiteralak, long[] adabegiKop) {
		...
		if(klausulaHutsaDago(formulaKopia)){
			return false;
		}
		if(formulaKopia.hutsaDa()){
			return true;
		}
...
//		}
		...
		literalarenBalioaFormulanJarri(formulaKopia,literala,baliogabekoLiteralakKopia);
		System.out.println(formulaKopia.toString());
		...
		if(!formulaBeteta){
			formulaKopia = formulaKopiatu(formula);
			literala.setEsleitutakoBalioa(false);
			adabegiKop[0]++;
			literalarenBalioaFormulanJarri(formulaKopia,literala,baliogabekoLiteralakKopia);
			System.out.println(formulaKopia.toString());
