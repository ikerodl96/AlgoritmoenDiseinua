...
public class TxanponKonbinazioak_01 {
	 private int L;
	 private int[] balioak;
	 private int n;
	 
	public TxanponKonbinazioak_01(int l, int[] balioak) {
		L = l;
	...
	public void txanponKonbinazioak(int SPB, int t, int[] Konbi){
		if(L-SPB==0){
			...
			if((L-SPB) >= minimoa){
				int zenbat = (L-SPB)/balioak[t];
				for(int i=0; i<=zenbat; i++){
					txanponKonbinazioak(SPB+(i*balioak[t]), t+1, Konbi);
				}
			}
		}
	}
